...
func (p *PortalOptions) Certificate() *Certificate {
	cert := C.telco_portal_options_get_certificate(p.opts)
	return &Certificate{cert}
}

// Token returns the token for the portal.
func (p *PortalOptions) Token() string {
	return C.GoString(C.telco_portal_options_get_token(p.opts))
}

// ACL returns the acls for the portal.
func (p *PortalOptions) ACL() []string {
	var sz C.gint
	arr := C.telco_portal_options_get_acl(p.opts, &sz)
...
func (p *PortalOptions) SetCertificate(certPath string) error {
	...
	C.telco_portal_options_set_certificate(p.opts, cert.cert)
	return nil
}
...
func (p *PortalOptions) SetToken(token string) {
	...
	C.telco_portal_options_set_token(p.opts, tokenC)
}
...
func (p *PortalOptions) SetACL(acls []string) {
	...
	C.telco_portal_options_set_acl(p.opts, arr, C.gint(sz))
