...
func (f *Child) Envp() []string {
	var length C.gint
	arr := C.telco_child_get_envp(f.child, &length)
...
func (f *Child) Clean() {
	clean(unsafe.Pointer(f.child), unrefTelco)
}
