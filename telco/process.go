...
func (p *Process) PID() int {
	if p.proc != nil {
		return int(C.telco_process_get_pid(p.proc))
	}
	return -1
}
...
func (p *Process) Name() string {
	if p.proc != nil {
		return C.GoString(C.telco_process_get_name(p.proc))
	}
	return ""
}
...
func (p *Process) Params() map[string]any {
	if p.proc != nil {
		ht := C.telco_process_get_parameters(p.proc)
...
func (p *Process) Clean() {
	if p.proc != nil {
		clean(unsafe.Pointer(p.proc), unrefTelco)
	}
}
