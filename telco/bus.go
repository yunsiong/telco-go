...
func (b *Bus) IsDetached() bool {
	dt := C.int(C.telco_bus_is_detached(b.bus))
	return dt == 1
}
...
func (b *Bus) Attach() error {
	var err *C.GError
	C.telco_bus_attach_sync(b.bus, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (b *Bus) Post(msg string, data []byte) {
	...
	C.telco_bus_post(b.bus, msgC, gBytesData)
	runtime.KeepAlive(gBytesData)
}

// Clean will clean resources held by the bus.
func (b *Bus) Clean() {
	clean(unsafe.Pointer(b.bus), unrefTelco)
}
