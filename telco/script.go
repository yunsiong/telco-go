...
func (s *Script) IsDestroyed() bool {
	destroyed := C.telco_script_is_destroyed(s.sc)
...
func (s *Script) Load() error {
	...
	var err *C.GError
	C.telco_script_load_sync(s.sc, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (s *Script) Unload() error {
	var err *C.GError
	C.telco_script_unload_sync(s.sc, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (s *Script) Eternalize() error {
	var err *C.GError
	C.telco_script_eternalize_sync(s.sc, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (s *Script) Post(jsonString string, data []byte) {
	...
	if len(data) > 0 {
		...
		C.telco_script_post(s.sc, jsonStringC, gBytesData)
		runtime.KeepAlive(gBytesData)
	} else {
		C.telco_script_post(s.sc, jsonStringC, nil)
	}
}
...
func (s *Script) EnableDebugger(port uint16) error {
	var err *C.GError
	C.telco_script_enable_debugger_sync(s.sc, C.guint16(port), nil, &err)
	if err != nil {
		return &FError{err}
	}
...
func (s *Script) DisableDebugger() error {
	var err *C.GError
	C.telco_script_disable_debugger_sync(s.sc, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (s *Script) Clean() {
	clean(unsafe.Pointer(s.sc), unrefTelco)
}
...
func (s *Script) hijackFn(message string, data []byte) {
	if strings.Contains(message, "telco:rpc") {
