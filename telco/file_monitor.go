...
func NewFileMonitor(path string) *FileMonitor {
	...
	m := C.telco_file_monitor_new(pathC)
...
func (mon *FileMonitor) Path() string {
	return C.GoString(C.telco_file_monitor_get_path(mon.fm))
}

// Enable enables file monitoring.
func (mon *FileMonitor) Enable() error {
	var err *C.GError
	C.telco_file_monitor_enable_sync(mon.fm, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (mon *FileMonitor) Disable() error {
	var err *C.GError
	C.telco_file_monitor_disable_sync(mon.fm, nil, &err)
	if err != nil {
		return &FError{err}
	}
	return nil
...
func (mon *FileMonitor) Clean() {
	clean(unsafe.Pointer(mon.fm), unrefTelco)
}
